...
const displayMovements = function (movements, sort = false) {
  // innerHTML passes actual html code through as oppose to textContent that only pases through text
...
// Switch sorted from false to true and vice versa
let sorted = false;
...
const reccomendedFoodChart = dogs.map(
  dog => (dog.recommendedFood = Math.trunc(recommendedFood(dog.weight)))
);
...
console.log(
  `Sarahs dog is eating too ${
    recommendedFood(sarahsDog.weight) < sarahsDog.curFood ? 'much' : 'little'
  } food`
);
...
const ownersDontEatEnough = [];

const fatDogs = dogs
  .filter(dog => recommendedFood(dog.weight) < dog.curFood)
  .filter(dog => ownersEatTooMuch.push(dog.owners));

const skinnyDogs = dogs
  .filter(dog => recommendedFood(dog.weight) > dog.curFood)
  .filter(dog => ownersDontEatEnough.push(dog.owners));

console.log(ownersEatTooMuch.flat());
console.log(ownersDontEatEnough.flat());

const fatOwner = ownersEatTooMuch.flat();
const skinnyOwner = ownersEatTooMuch.flat();
...
console.log(`${fatOwner.join(' and ')}'s dogs eat too much!`);
console.log(`${skinnyOwner.join(' and ')}'s dogs dont eat enough!`);

// 5. Log to the console whether there is any dog eating EXACTLY the amount of food that is recommended (just true or false)
console.log(`${dogs.some(dog => dog.curFood === dog.recommendedFood)}`);

// 6. Log to the console whether there is any dog eating an OKAY amount of food (just true or false)
const closeEnough = dog =>
  dog.curFood > dog.recommendedFood * 0.9 &&
  dog.curFood < dog.recommendedFood * 1.1;

console.log(dogs.some(closeEnough));

// 7. Create an array containing the dogs that are eating an OKAY amount of food (try to reuse the condition used in 6.)
const goodBoys = dogs.filter(dog => closeEnough(dog));
...
const newDogs = dogs.map(dog => dog.recommendedFood).sort((a, b) => a - b);
console.log(newDogs);
...
// // SPLICE does mutate orignal array
...
// // REVERSE does mutate orignal array
// arr = ['a', 'b', 'c', 'd', 'e'];
...
// console.log('Isaac'.at(0));
// console.log('Isaac'.at(-1));
...
// // forEach passes the value, the index and the entire array
